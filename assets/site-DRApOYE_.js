import{f as g}from"./firebase-applet-config-Ck6_0kXN.js";import{initializeApp as E}from"https://www.gstatic.com/firebasejs/9.22.1/firebase-app.js";import{initializeFirestore as T,doc as B,onSnapshot as L,query as y,collection as u,where as m,getDocs as h}from"https://www.gstatic.com/firebasejs/9.22.1/firebase-firestore.js";async function M(){try{const p=E(g),b=T(p,{experimentalForceLongPolling:!0},g.firestoreDatabaseId),n=new URLSearchParams(window.location.search),l=n.get("id");if(n.get("preview")==="true"&&(document.getElementById("previewBanner").style.display="block"),window.applyTemplate=e=>{const t=document.getElementById("templateStyles"),i=document.getElementById("dynamicFont");!t||!i||(e==="Salon & Beauty"?(i.href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;1,700&family=Montserrat:wght@300;400&display=swap",t.innerHTML+=`
                       ...
                       `):e==="Universal Professional"&&(i.href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap",document.body.style.fontFamily="'Plus Jakarta Sans', sans-serif",t.innerHTML=`
                        ...
                        `))},window.renderSite=e=>{var x,w;if(!e)return;document.title=`${e.businessName||"Business"} | Official Site`,document.getElementById("siteLogoText").innerText=e.businessName||"Business",document.getElementById("siteTitle").innerText=e.businessName||"Headline",document.getElementById("siteTagline").innerText=e.tagline||"Leading Service Business in Nairobi",document.getElementById("aboutName").innerText=e.businessName||"Us",document.getElementById("aboutContent").innerText=e.aboutText||"We provide top-tier services to our clients with high attention to detail and professional excellence.",document.getElementById("footerName").innerText=e.businessName||"Business",document.getElementById("footerTagline").innerText=e.tagline||"",document.getElementById("copyName").innerText=e.businessName||"Business";const t=document.getElementById("siteHero");(x=e.images)!=null&&x.hero?t.style.backgroundImage=`url('${e.images.hero}')`:t.style.backgroundColor="#1a1a1a";const i=document.getElementById("servicesGrid");if(i.innerHTML="",e.services&&e.services.length>0?e.services.forEach(s=>{const o=document.createElement("div");o.className="card",o.innerHTML=`
                            ...
                            `).join("")):s.style.display="none";const v=document.getElementById("pricing"),S=document.getElementById("pricingGrid");(d.includes("Pro")||d.includes("Enterprise")||c.includes("Pricing"))&&e.pricing&&e.pricing.length>0?(v.style.display="block",S.innerHTML=e.pricing.map((a,$)=>`
                                <div class="card" style="position: relative; ${$===1?"border-color: var(--primary);":""}">
                                    ...
                                    <ul style="list-style: none; padding: 0; margin-bottom: 30px;">
                                        ${a.features.map(H=>`<li style="margin-bottom: 10px; font-size: 0.9rem; opacity: 0.8; display: flex; align-items: center; gap: 10px;"><i data-lucide="check-circle-2" style="width: 14px; color: #22c55e;"></i> ${H}</li>`).join("")}
                                    </ul>
                                    <a href="${window.currentWaUrl||"#"}" class="btn btn-primary" style="width: 100%; text-align: center;">Choose Plan</a>
                                </div>
                            `).join("")):v.style.display="none";const k=document.getElementById("testimonials"),P=document.getElementById("testimonialsGrid");(d!=="Starter Presence"||c.includes("Testimonials"))&&e.testimonials&&e.testimonials.length>0?(k.style.display="block",P.innerHTML=e.testimonials.map(a=>`
                                <div class="card">
                            ...
                            `).join("")):k.style.display="none";const I=document.getElementById("cta");(d!=="Starter Presence"||c.includes("CTA"))&&e.cta?(I.style.display="block",e.cta.title&&(document.getElementById("ctaTitle").innerText=e.cta.title),e.cta.btn&&(document.getElementById("ctaBtn").innerText=e.cta.btn)):I.style.display="none"}else document.getElementById("features").style.display="none",document.getElementById("pricing").style.display="none",document.getElementById("testimonials").style.display="none",document.getElementById("cta").style.display="none";window.lucide&&window.lucide.createIcons()},l){console.log("Loading Site ID:",l);const e=B(b,"clientSites",l);L(e,t=>{console.log("Snapshot received. Exists:",t.exists()),t.exists()?renderSite(t.data()):document.body.innerHTML=`
                            ...
                            </div>`})}else console.log("No Site ID found. Attempting Domain Lookup..."),window.domainLookup&&window.domainLookup()}catch(p){console.error("Initialization Error:",p),document.body.innerHTML=`<div style="padding: 100px; text-align:center; color:white;"><h1>System Initialization Failed</h1><p>${p.message}</p></div>`}}M();async function z(){if(new URLSearchParams(window.location.search).get("id"))return;const n=window.location.hostname;console.log("[Domain Lookup] Evaluating host:",n);try{const l=E(g),f=T(l,{experimentalForceLongPolling:!0},g.firestoreDatabaseId),e=y(u(f,"clientSites"),m("status","==","Live"),m("custom_domains","array-contains",n));let t=await h(e);if(t.empty){const i=y(u(f,"clientSites"),m("status","==","Live"),m("customDomain","==",n));t=await h(i)}if(t.empty&&(n.includes(".quicksitekenya.co.ke")||n.includes(".quicksite.co.ke"))){const r=n.split(".")[0];if(r!=="www"&&r!=="quicksitekenya"&&r!=="quicksite"){console.log("[Domain Lookup] Trying subdomain match:",r);const c=y(u(f,"clientSites"),m("status","==","Live"),m("subdomain","==",r));t=await h(c)}}if(t.empty)console.warn("[Domain Lookup] No matching client found for:",n),document.title="Site Not Found | QuickSite Kenya",document.body.innerHTML=`
                    ...
                    `;else{const i=t.docs[0];console.log("[Domain Lookup] Match found:",i.id),L(B(f,"clientSites",i.id),r=>{window.renderSite&&window.renderSite(r.data())})}}catch(l){console.error("[Domain Lookup] Error:",l)}}window.domainLookup=z;
